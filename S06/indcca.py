...
class Adversario_Identidade(ABC):
    ...
    def guess(self, enc_oracle: callable, dec_oracle: callable, ciphertext: bytes) -> bool:
...
def IND_CCA(C,A):
    ...
    def dec_oracle(ctxt):
        if ctxt == desafio_c:
            return "ERRO: Não podes decifrar o desafio".encode('utf-8')
        return cif.dec(k, ctxt)
    ...
    # print(b)
    if b==0:
        desafio_c = cif.enc(k,m0)
    else:
        desafio_c = cif.enc(k,m1)
    d = adv.guess(enc_oracle,dec_oracle, desafio_c)
    # print(d,b==d)
    return b==d
...
if __name__ == "__main__":
    ...
    for i in range(jogos):
        acertou = IND_CCA(CipherCCA,Adversario_CCA)
        if acertou:
            vitorias += 1
    ...
    for i in range(jogos2):
        acertou = IND_CCA(IdentityCipher,Adversario_Identidade)
        if acertou:
            vitorias2 += 1
